...
pub const INTERNAL_RAM0: usize = 8 * 1024; // 8kB of Internal RAM
pub const INTERNAL_RAM1: usize = 0x7F; // More Internal RAM
...
impl Memory {
  pub fn get(&self, pos: u16) -> u8 {
    if (0x0000..0x8000).contains(&pos) { // 32kB Cartridge
      ...
      self.internal_ram0[pos as usize - 0xC000]
    } else if (0xE000..0xFE00).contains(&pos) { // Echo of 8kB Internal RAM
      self.internal_ram0[pos as usize - 0xE000]
      ...
      self.io.unusable_io_ram0[pos as usize - 0xFEA0]
    } else if (0xFF00..0xFF4C).contains(&pos) { // I/O Ports
      self.io.io_ports[pos as usize - 0xFF00]
    } else if (0xFF4C..0xFF80).contains(&pos) { // Empty but unusable for I/O
      self.io.unusable_io_ram1[pos as usize - 0xFF4C]
    } else if (0xFF80..0xFFFF).contains(&pos) { // More Internal RAM
      self.internal_ram1[pos as usize - 0xFF80]
